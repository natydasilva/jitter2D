#' Jittered 2D points based on Sobolev local sequence
#'
#' The jitter geom is a convenient shortcut for
#' `geom_point(position = "jitterquasiloc")`. It adds a small amount of random
...
#' @inheritParams position_jitter_quasiloc
#' @seealso
...
#' ggplot(mpg, aes(x=cty, y=hwy)) + geom_jitter_quasiloc()
#'
geom_jitter_quasiloc <- function(
  mapping = NULL,
  data = NULL,
  stat = "identity",
  ...
  if (!missing(weight)) {
    ...
    position <- position_jitter_quasiloc(weight = weight)
  }
