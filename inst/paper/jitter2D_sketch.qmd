...
For various reasons it is useful for spreading points in dotplot or scatterplot displays. When points are overplotted in a display the assessment of the distribution or association between variables may not be accurately interpreted. There is a long history of using dotplots for examining distributions, with a variety of recommended methods for spreading apart identical points. @Wilkinson01081999 provides a simple and comprehensive discussion. Dotplots for studying distributions provide a 
...
@Wilkinson01081999 references Tukey and Tukey (1990) ans describes some variety in jittered dotplots.
...
library(stringr)
library(here)
source(here('inst/sandbox/utils.R') )
devtools::load_all()

# function to plot toy examples data 
...
# function to plot real data examples 
realdata_fn <- function(df, bw) {
  
  pp <- vector(mode = 'list', length = 6)
  
  pl <- df |> ggplot( aes(x,y) ) + geom_jitter() 
  pl <- ggplot_build(pl)

  pp[[1]] <- df
  pp[[2]] <- pl@data[[1]][, c('x', 'y')]
  pr <- expand.grid(n = c('random', 'quasi'), d = c('local', 'global'))
  for (p in 1:4) {
    pp[[p + 2]] <- compute_jitter2D(df, noise = pr$n[p], dir = pr$d[p], bw=bw)
  }
  ...
  names(pp) <- c('1:Original', '4:geom_jitter()', pr_nm)
  dd <- bind_rows(pp, .id = 'type')
...
### Education example 
```{r 'lb'}
load(here('data/data_LB2021_uy.rda'))

df <- dt_LB21 |> 
  filter( Departamento == 'MALDONADO', 
  Contexto_Sociocultural == 'Quintil 1' ) |>
    drop_na( correctas ) |> 
    select(dias, correctas) |> 
    setNames(nm = c('x', 'y') ) |> 
    scale() |> data.frame()
...
### Mother age and alive previuos childs

```{r}
load(here('data/alive_ch.rda'))

df <- dat |> 
  filter( tipo_prestador_salud == 'Seguro Privado' ) |> 
  select(alive_previous_child, edad_materna) |> 
      setNames(nm = c('y', 'x') ) 
